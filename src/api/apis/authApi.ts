...
import { LoginRequestDto, RefreshTokenRequestDto, TokenResponseDto, TokensApiLoginPostRequest, TokensApiRefreshTokenPostRequest, TokensApiRegisterPostRequest, UserRoleEnum } from '../api';
...
export const TokensApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
        ...
        refreshTokenPost: async (refreshTokenRequestDto?: RefreshTokenRequestDto, options: any = {}): Promise<RequestArgs> => {
            const localVarPath = `/api/user/renewrefreshtoken`;
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
            if (configuration) {
